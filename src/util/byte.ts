...
export class ByteReader {
	...
	read(type: Type): boolean | number | bigint | string {
		switch (type) {
			...
			case Type.SHORT:
				return Buffer.from(this.read_bytes(2)).readInt16BE();
			case Type.INTEGER:
				return Buffer.from(this.read_bytes(4)).readInt32BE();
			case Type.FLOAT:
				return Buffer.from(this.read_bytes(4)).readFloatBE();
			case Type.DOUBLE:
				return Buffer.from(this.read_bytes(8)).readDoubleBE();
			case Type.LONG:
				return Buffer.from(this.read_bytes(8)).readBigInt64BE(); // longs are bing int 64
	...
	read_bytes(size: number) {
		if (this._cursor + size > this._bytes.length) throw new Error("Index out of bounds");
...
export class ByteWriter {
	...
	write(type: Type, value: boolean | number | bigint | string): ByteWriter {
		switch (type) {
			...
			case Type.BYTE:
			case Type.UNSIGNED_BYTE:
				this._bytes.push((value as number) & 255);
				break;
