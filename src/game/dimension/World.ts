...
import { parse_nbt_file } from "../../util/nbt.ts";

export default class World {
	private readonly _chunks: Chunk[];

	private readonly world_path: string;
	...
	private load() {
		try {
			...
			const level_nbt = parse_nbt_file(
				Deno.readFileSync(this.world_path + "/level.dat"),
			);
			// TODO
		} catch (e) {
			console.error(
				"Failed to load world " + DimensionType[this.dimension] +
					", loading default...",
			);
